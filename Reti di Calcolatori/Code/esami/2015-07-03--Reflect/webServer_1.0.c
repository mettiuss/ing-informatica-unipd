...
int main() {
  ...
  while (1) {
    ...
    if (!strncmp(url, "/gateway/", 9)) {
      ...
      filename = "output";
    } else if (!strncmp(url, "/reflect", 8)) {
      char* response_200 = "HTTP/1.0 200 OK\r\n\r\n";
      write(sock2, response_200, strlen(response_200));

      char status_line[1000];
      sprintf(status_line, "%s %s %s\r\n", method, url, version);
      write(sock2, status_line, strlen(status_line));

      for (int i = 1; i < j - 1; i++) {
        char header[1000];
        sprintf(header, "%s: %s\r\n", h[i].name, h[i].value);
        write(sock2, header, strlen(header));
      }
      ...
      write(sock2, ipClient, strlen(ipClient));
      close(sock2);
      continue;
    }
