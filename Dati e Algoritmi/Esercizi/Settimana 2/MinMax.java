...
public class MinMax {
    ...
    public static double[] findMinMax(double[] a) {
        ...
        if (a.length % 2 == 1) {
            mins[mins.length - 1] = maxs[maxs.length - 1] = a[a.length - 1]; // se lunghezza dispari...
        }

        // Trovo ora il minimo e il massimo a partire dagli array di mins e max,
        // l'operazione richiede n confronti
        double min = mins[0];
        double max = maxs[0];
    ...
    public static void main(String[] args) {
        ...
        System.out.println(
            String.format(
                "The minimum value in the array is %s and the maximum is %s",
                result[0],
                result[1]
            )
        );
